...
a example GWAS was generated for two cohorts. Variant 99 has a true
effect on the simulated phenotype, explaining 1% of the phenotypic
variance.
...
<img src="FINEMAP-MISS_files/figure-markdown_strict/read data-1.png" width="90%" />

Let’s simulate missing data by removing values for 20% of variants from
one study.
...
Let’s meta-analyze the summary statistics and visualize the p-values of
...
<img src="FINEMAP-MISS_files/figure-markdown_strict/meta_p-1.png" width="90%" />

# Fine-mapping

If we try to fine-map this partially missing data with FINEMAP, we
cannot detect the true causal variant.


    FM_output <- original_FINEMAP(ses = ses, betas = betas, R = LD, n_studies = 2,
                                  freqs = MAF, variant_sample_sizes = variant_sample_sizes)
    #> [1] "Creating sample overlap matrix"
    #> [1] "Creating storage"
    #> [1] "Finemapping"
    #> [1] "Calculating pips"
    #> [1] "Credible Sets"
    plot(FM_output$pips, ylab = "PIP", main = "FINEMAP PIPs")
    points(toydata_FINEMAPMISS$causal_snp, FM_output$pips[toydata_FINEMAPMISS$causal_snp],
           pch = 21, bg = "green")
    points(FM_output$cred[,1], FM_output$pips[FM_output$cred[,1]],
           col = "red")
    legend("topleft", pch = c(21,1), legend = c("Causal variant", "Credible set"), pt.bg = c("green", NA),
           col = c("black", "red"))

<img src="FINEMAP-MISS_files/figure-markdown_strict/FINEMAP-1.png" width="90%" />

## FINEMAP-MISS

With FINEMAP-MISS, the true causal variant is included in the credible
set.
    ...
    FMM_output <- FINEMAPMISS(ses = ses, betas = betas, R = LD, 
                              ...
                              scaled_data = T)
    #> [1] "Creating sample overlap matrix"
    #> [1] "Inverting covariance matrix"
    #> [1] "Pre-multiplication matrices for fine-mapping"
    #> [1] "Creating storage"
    #> [1] "Finemapping"
    #> [1] "2% complete, config size: 1, new mass: 0, number of configs evaluated: 1"
    #> [1] "4% complete, config size: 1, new mass: 1, number of configs evaluated: 238"
    #> [1] "6% complete, config size: 1, new mass: 0.002, number of configs evaluated: 475"
    #> [1] "8% complete, config size: 1, new mass: 0.01, number of configs evaluated: 712"
    #> [1] "10% complete, config size: 1, new mass: 0.007, number of configs evaluated: 949"
    #> [1] "12% complete, config size: 2, new mass: 0.004, number of configs evaluated: 1186"
    #> [1] "Calculating pips"
    #> [1] "Credible Sets"
    plot(FMM_output$pips, ylab = "PIP", main = "FINEMAP-MISS PIPs")
    points(toydata_FINEMAPMISS$causal_snp, FMM_output$pips[toydata_FINEMAPMISS$causal_snp],
           pch = 21, bg = "green")
    points(FMM_output$cred[,1], FMM_output$pips[FMM_output$cred[,1]],
           col = "red")
    legend("topleft", pch = c(21,1), legend = c("Causal variant", "Credible set"), pt.bg = c("green", NA),
           col = c("black", "red"))

<img src="FINEMAP-MISS_files/figure-markdown_strict/FINEMAP-MISS-1.png" width="90%" />
...
We can alternatively impute missing data and then run FINEMAP.
    ...
    plot(FM_imputed_output$pips, ylab = "PIP", main = "FINEMAP PIPs")
    points(toydata_FINEMAPMISS$causal_snp, FM_imputed_output$pips[toydata_FINEMAPMISS$causal_snp],
           pch = 21, bg = "green")
    points(FM_imputed_output$cred[,1], FM_imputed_output$pips[FM_imputed_output$cred[,1]],
           col = "red")
    legend("topleft", pch = c(21,1), legend = c("Causal variant", "Credible set"), pt.bg = c("green", NA),
           col = c("black", "red"))

<img src="FINEMAP-MISS_files/figure-markdown_strict/FINEMAP imputed-1.png" width="90%" />
